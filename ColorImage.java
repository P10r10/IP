...
class ColorImage {
	...
	for (int i = 0; i < this.getHeight(); i++)
		for (int j = 0; j < this.getWidth(); j++)
			this.setColor(j, i, Color.inverted(this.getColor(j, i)));
	}
	/* completar isto
	void copyImage(ColorImage this){
		ColorImage img = this.copyImage();
		for (int i = 0; i < this.getHeight(); i++)
			for (int j = 0; j < this.getWidth(); j++)
				this.setColor(j, i, img.getColor(j, i));
	}*/
	
	ColorImage reflectImage(){
		ColorImage res = new ColorImage(this.getWidth(), this.getHeight());
		for (int i = 0; i < this.getHeight(); i++)
			for (int j = 0; j < this.getWidth(); j++)
				res.setColor(this.getWidth() - j - 1, i, this.getColor(j, i));
		return res;
		}
}